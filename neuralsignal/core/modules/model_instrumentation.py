...
def generate_from_batch(
    ...
    try:
        tokenizer_options = {
            ...
            "truncation": truncation_length > 0,
        }
        if truncation_length > 0:
            tokenizer_options["max_length"] = truncation_length
        encoded = tokenizer(input, **tokenizer_options)
        input_ids = encoded.input_ids.to(model.device)
        attention_mask = getattr(encoded, "attention_mask", torch.ones_like(input_ids)).to(model.device)
        aborted = False
        try:
        ...
        except NSAbortLLM as error:
            logging.debug("LLM execution aborted %s", error)
            aborted = True
        ...
        for batch_idx in range(batch_size):
            decoded_output = "ABORT" if aborted else tokenizer.decode(
            ...
            if model_instrumented:
                gi.add_data(hc.get_data_by_batch_index(batch_idx))
            results.append(gi)
        return results
    finally:
        if model_instrumented:
...
def get_model_type(model) -> str:
    ...
    if not isinstance(model, str):
        configured_type = getattr(getattr(model, "config", None), "model_type", None)
        if configured_type in {"t5", "longt5"}:
...
def instrument_model(cfg, model, hc: Collector) -> list:
    ...
    if type == "deberta":
        return instrument_deberta(cfg, model, hc)
    if type == "bert":
...
def instrument_deberta(cfg, model, hc: Collector) -> list:
    # Keep a list of these so we can de-instrument the model later
    registered_hooks = []
    if cfg["instrument_encoder"]:

        for i, lyr in enumerate(model.encoder.layer):
            add_hook(
                lyr.attention.self.query_proj, hc, registered_hooks)
            lyr.attention.self.query_proj.ns_name = "encoder." + \
                f".self_attention.query_proj_{i}"
            add_hook(
                lyr.attention.self.key_proj, hc, registered_hooks)
            lyr.attention.self.key_proj.ns_name = "encoder." + \
                f".self_attention.key_proj_{i}"
            add_hook(
                lyr.attention.self.value_proj, hc, registered_hooks)
            lyr.attention.self.value_proj.ns_name = "encoder." + \
                f".self_attention.value_proj_{i}"
